...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (ClientPrefs.shaders)
			FlxG.camera.setFilters([new ShaderFilter(pibbyFNF)]);
		...
		hitbox = new FlxSprite().makeGraphic(492, 285, FlxColor.TRANSPARENT);
		add(hitbox);
		...
		arrowL.x = arrowL.x - 246 - 80;
		add(arrowL);
		arrowL.scale.set(4, 4);
		arrowL.blend = ADD;
		if (ClientPrefs.shaders)
			arrowL.shader = bloomFNF;

		arrowR = new FlxSprite().loadGraphic(Paths.image('fpmenu/arrowL'));
		arrowR.screenCenter(X);
		
		arrowR.x = arrowR.x + 246 + 80;
		arrowR.flipX = true; // just one image
		arrowR.antialiasing = ClientPrefs.globalAntialiasing;
		add(arrowR);
		arrowR.scale.set(4, 4);
		arrowR.blend = ADD;
		if (ClientPrefs.shaders)
			arrowR.shader = bloomFNF;
	...
	override function update(elapsed:Float)
	{
		...
		var leftP = BSLTouchUtils.apertasimples(arrowL);
		var rightP = BSLTouchUtils.apertasimples(arrowR);
		var accepted = BSLTouchUtils.apertasimples(hitbox);
		...
		if (controls.UI_UP_P)
		{
			...
			if (ClientPrefs.shaders)
				FlxG.camera.setFilters([new ShaderFilter(pibbyFNF), new ShaderFilter(glitchFWFNF)]);
			var gameObjects = [
				bg, arrowL, arrowR, image, stagebox, stagebox_L, stagebox_R, threat, levelBarBG, gradient
			];
		...
		hitbox.y = stagebox.y + 176;
		hitbox.x = stagebox.x + 393;
		arrowR.y = stagebox.y + 319; // y + 319 = image center
		arrowL.y = stagebox.y + 319;
	}
